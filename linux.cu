			...
			for (int i = 0; i < 3; i++)
			{
				for (int j = 0; j < scale_factor; j++)
				{
					for (int k = 0; k < scale_factor; k++)
					{
						...
						(*image_H).at<Vec3b>(scale_factor * row + j, scale_factor * col + k)[2-i] = (uchar)(data < 1.f ? data * 255.f : 255);//opencv图像的存储格式为BGR
					}
				}
			}
		}
...
int main(int argc, char** argv)
{
  ...
  // cudnn_tion_fwd_algo_winograd——它使用Winograd算法执行卷积。
	...
	//cudaMemset(d_featuremap_2, 0, size_of_featuremap);
	/*
	float h_kernel[128][18][3][3]; // NCHW
