...
int main(int argc, char** argv)
{
  ...
  // cudnn_tion_fwd_algo_winograd——它使用Winograd算法执行卷积。
	cudnnConvolutionFwdAlgoPerf_t convolution_algorithm_first;
	checkCUDNN(
		cudnnGetConvolutionForwardAlgorithm_v7(handle,
			...
			CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_PRECOMP_GEMM, // CUDNN_CONVOLUTION_FWD_SPECIFY_​WORKSPACE_LIMIT（在内存受限的情况下，memoryLimitInBytes 设置非 0 值）
  ...
  // cudnn_tion_fwd_algo_fft——它使用快速傅立叶变换(FFT)进行卷积或
  // cudnn_tion_fwd_algo_winograd——它使用Winograd算法执行卷积。
	cudnnConvolutionFwdAlgoPerf_t convolution_algorithm_mid;
	checkCUDNN(
		cudnnGetConvolutionForwardAlgorithm_v7(handle,
  ...
  // cudnn_tion_fwd_algo_fft——它使用快速傅立叶变换(FFT)进行卷积或
  // cudnn_tion_fwd_algo_winograd——它使用Winograd算法执行卷积。
	cudnnConvolutionFwdAlgoPerf_t convolution_algorithm_last;
	checkCUDNN(
		cudnnGetConvolutionForwardAlgorithm_v7(handle,
	...
	checkCUDNN(cudnnGetConvolutionForwardWorkspaceSize(handle,
		...
		convolution_descriptor_mid,
		output_descriptor_mid,
		convolution_algorithm_mid.algo,
		&workspace_bytes));
	...
	for (int item = 0; item < imageSet.size(); item++)
	{
		...
		checkCUDNN(cudnnConvolutionForward(handle,
			...
			d_kernel[0],
			convolution_descriptor_first,
			convolution_algorithm_first.algo,
		...
		checkCUDNN(cudnnConvolutionForward(handle,
			...
			convolution_descriptor_last,
			convolution_algorithm_last.algo,
			d_workspace, // 注意，如果我们选择不需要额外内存的卷积算法，d_workspace可以为nullptr。
			workspace_bytes,
			&beta,
			output_descriptor_last,
